...
const GoldView: React.FC<GoldViewProps> = ({ onStageClick, onNavigateToAlert }) => {
  ...
  return (
    <div className="space-y-6 pb-12 animate-fade-in font-sans">
      ...
      {/* --- SECTION 1: COMMAND CENTER & INTELLIGENCE --- */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 min-h-[400px]">
         
         {/* LEFT: ALERTS & QUEUE (Span 8) */}
         <div className={`${glassCard} lg:col-span-8 flex flex-col`}>
             {/* Tab Header */}
         ...
         {/* RIGHT: SYSTEM INFO & AI INSIGHT (Span 4) */}
         <div className="lg:col-span-4 flex flex-col gap-6">
             
             {/* 1. System Status & Date (Moved here) */}
             <div className="flex items-center justify-between bg-[var(--color-surface)]/50 backdrop-blur-sm p-4 rounded-2xl border border-[var(--color-border)]">
                 <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full border text-xs font-bold ${isSystemHealthy(healthData?.status) ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-500' : 'bg-rose-500/10 border-rose-500/20 text-rose-500'}`}>
             ...
             {/* 2. AI Insight Card */}
             <div className={`${glassCard} p-6 flex-1 flex flex-col relative overflow-hidden`}>
                {/* Ambient Background */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        {/* MINI STATS (Yield & Production Rate) (Span 4) */}
